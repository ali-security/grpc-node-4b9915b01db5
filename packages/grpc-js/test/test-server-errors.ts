...
describe('Other conditions', () => {
  ...
  describe('Max message size', () => {
    ...
    it('Should be enforced on the client before sending an oversized message', done => {
      const limitedClient = new testServiceClient(
        `localhost:${port}`,
        clientInsecureCreds,
      ...
      limitedClient.unary(
        ...
        (error?: ServiceError) => {
          ...
          assert.match(
            error.details,
            /Attempted to send message with a size larger/
          );
          limitedClient.close();
          done();
        }
